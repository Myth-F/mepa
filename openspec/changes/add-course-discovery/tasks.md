...
- [x] 1.5 Add unit/integration tests for document build, removal on unpublish, and recompute equivalence
...
- [x] 2.3 Add unit/integration tests for ranking, accent/case insensitivity, facet filtering, published-only results, and empty-result behavior
...
- [x] 3.3 Verify keyboard operation and non-color-only relevance; catalogue uses indexed queries + pagination (request-rendered because results depend on URL search params)
- [x] 3.4 Add Playwright coverage for search, facet filtering, sorting, pagination, shareable URL state, and no-results
...
- [x] 4.5 Add Playwright coverage for showcase rendering, first-visit variation, non-blocking open + invitation, and returning-visitor continuation

## 5. Authoring Integration

- [x] 5.1 Add classification (category, tags, level, duration, language) + featured curation to authoring
- [x] 5.2 Add tests for classification persistence, featured curation, and that publication refreshes the discovery document

## 6. Discoverability, Privacy And Verification

- [x] 6.1 Add public category pages and a sitemap of published modules and categories; set canonical metadata
...
- [x] 6.3 Run an RGAA check on the search form, facets, showcase and no-results state; record results
- [x] 6.4 Run the full automated suite and production build (green), and document the search ranking, facets, featured curation, and recompute
